...
func NewDMDriver() Driver {
	return newSQLQueryDriver("dm8", "dm", introspectDM, dmVersion)
}

func dmVersion(ctx context.Context, db *sql.DB) (string, error) {
	return readSingleString(ctx, db, "SELECT BANNER FROM V$VERSION WHERE ROWNUM = 1")
}

func introspectDM(ctx context.Context, db *sql.DB) (*Metadata, error) {
	schema, err := dmCurrentSchema(ctx, db)
	if err != nil {
		return nil, err
	}
	tables, err := dmTables(ctx, db, schema)
	if err != nil {
		return nil, err
	}
	columnsByTable, err := dmColumns(ctx, db, schema)
	if err != nil {
		return nil, err
	}
	indexesByTable, err := dmIndexes(ctx, db, schema)
	if err != nil {
		return nil, err
	}
	foreignKeysByTable, err := dmForeignKeys(ctx, db, schema)
	if err != nil {
		return nil, err
	}
...
func dmCurrentSchema(ctx context.Context, db *sql.DB) (string, error) {
	queries := []string{
	...
	for _, query := range queries {
		schema, err := readSingleString(ctx, db, query)
		if err == nil && strings.TrimSpace(schema) != "" {
			return strings.ToUpper(strings.TrimSpace(schema)), nil
		}
		if err != nil {
			lastErr = err
		}
	}
	return "", fmt.Errorf("read dm current schema: %w", lastErr)
}

func dmTables(ctx context.Context, db *sql.DB, schema string) ([]Table, error) {
...
ORDER BY table_schema, table_name`, schema, schema)
	if err != nil {
		return nil, fmt.Errorf("query dm tables: %w", err)
	}
	defer rows.Close()
	return scanTableRows(rows, "dm8")
}

func dmColumns(ctx context.Context, db *sql.DB, schema string) (map[string][]Column, error) {
	rows, err := db.QueryContext(ctx, `
SELECT c.owner,
       c.table_name,
       c.column_name,
       ...
       c.data_type ||
         CASE
           WHEN c.data_type IN ('VARCHAR', 'VARCHAR2', 'NVARCHAR2', 'CHAR', 'NCHAR') THEN '(' || c.char_length || ')'
...
ORDER BY c.owner, c.table_name, c.column_id`, schema)
	if err != nil {
		return nil, fmt.Errorf("query dm columns: %w", err)
	}
	defer rows.Close()

	columnsByTable := map[string][]Column{}
	for rows.Next() {
		var tableSchema, tableName, columnName, dataType, columnType string
		var ordinal int
		var nullable, defaultValue, columnKey, foreignKey, comment sql.NullString
		if err := rows.Scan(&tableSchema, &tableName, &columnName, &ordinal, &dataType, &columnType, &nullable, &defaultValue, &columnKey, &foreignKey, &comment); err != nil {
			return nil, fmt.Errorf("scan dm column: %w", err)
		}
		...
		columnsByTable[key] = append(columnsByTable[key], Column{
			...
			Nullable:        strings.EqualFold(nullable.String, "Y"),
	...
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("iterate dm columns: %w", err)
	}
	return columnsByTable, nil
}

func dmIndexes(ctx context.Context, db *sql.DB, schema string) (map[string][]Index, error) {
	rows, err := db.QueryContext(ctx, `
SELECT i.owner,
       i.table_name,
       i.index_name,
...
ORDER BY i.owner, i.table_name, i.index_name, c.column_position`, schema)
	if err != nil {
		return nil, fmt.Errorf("query dm indexes: %w", err)
	}
	defer rows.Close()
	return scanOracleStyleIndexes(rows, "dm8")
}

func dmForeignKeys(ctx context.Context, db *sql.DB, schema string) (map[string][]ForeignKey, error) {
	rows, err := db.QueryContext(ctx, `
SELECT ac.owner,
       acc.table_name,
       ac.constraint_name,
...
ORDER BY ac.owner, acc.table_name, ac.constraint_name, acc.position`, schema)
	if err != nil {
		return nil, fmt.Errorf("query dm foreign keys: %w", err)
	}
	defer rows.Close()
	return scanOracleStyleForeignKeys(rows, "dm8")
}
