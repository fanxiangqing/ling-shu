...
- 跨会话用户长期记忆：保存稳定的个人偏好、职责和约定，支持显式记住/忘记、自动提取、项目级覆盖、语义召回和个人管理界面。
...
用户可以在对话中说“请记住以后默认用柱状图”“你记得我什么”“忘记我的图表偏好”，也可以从左侧导航打开“我的记忆”弹窗进行添加、编辑、删除和清空。

- 显式保存和通过安全、置信度校验的自动提取记忆都会立即生效，并可由用户随时编辑或删除。
