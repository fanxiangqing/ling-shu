...
```mermaid
flowchart TD
    A["项目管理员<br/>创建内嵌应用"] --> B["Ling-Shu<br/>生成 app_id / app_secret"]
    B --> C["三方后端<br/>保存 app_secret"]
    B --> D["三方前端<br/>加载 JS SDK"]
    D --> E["LingShuEmbed.init<br/>appId + key + position + tokenProvider"]
    E --> F["页面出现悬浮机器人"]
    F --> G["用户点击机器人"]
    G --> H["SDK 调用 tokenProvider"]
    H --> I["三方后端读取登录态<br/>external_user_id / external_user_name"]
    I --> J["POST /api/v1/embed/token"]
    C --> J
    J --> K["Ling-Shu 校验 app_secret<br/>签发短期 embed token"]
    K --> L["SDK 创建 iframe<br/>/embed/:app_id?key=...#token=..."]
    L --> M["iframe 调用<br/>/api/v1/embed/bootstrap"]
    M --> N{"会话策略"}
    N -->|user| O["按 app_id + external_user_id<br/>复用默认会话"]
    N -->|context| P["按 app_id + external_user_id + key<br/>复用业务上下文会话"]
    N -->|new| Q["每次 bootstrap<br/>创建新会话"]
    O --> R["绑定或创建 chat_session"]
    P --> R
    Q --> R
    R --> S["返回数据源 / 会话 ID<br/>ASR/TTS 能力"]
    S --> T["文本问数 SSE"]
    S --> U["实时语音 WebSocket"]
    T --> V["嵌入 iframe 展示结果"]
    U --> V
